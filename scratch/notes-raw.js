export const toneNoteCsv = `note,frequency,wavelength
C0,16.35,2109.89
...
export const noteTones = (csv) => {
  const data = csv.trim().split('\n');
  const props = data.shift().trim().split(',');
  const dataBody = data;
  return dataBody.map((row, rowIndex) => {
    const newNote = row.trim().split(',')
      .reduce((acc, curr, fieldIndex) => {
        !acc.id ? acc.id = +rowIndex : null;
        !acc.step ? acc.step = +rowIndex % 12 : null;

        if (!acc.note && props[fieldIndex] === 'note') acc.note = curr.replace(/[0-9]/g, '');
        if (!acc.name && props[fieldIndex] === 'note') acc.name = curr;
        if (acc.note && acc.note.includes('#')) {
          const split = acc.note.split('/')
          acc.isNatural = false;
          acc.sharp = split[0]
          acc.flat = split[1]
        } else acc.isNatural = true;

        if (props[fieldIndex] === 'wavelength' || props[fieldIndex] === 'frequency') acc[props[fieldIndex]] = +curr;
        return acc;
      }, {});

    newNote.octave = +(newNote.name.replace(/[^0-9]+/g, '')[0])
    return newNote
  })
};

// localStorage.setItem('noteData', JSON.stringify(noteTones(toneNoteCsv)))
// console.log('tobes', localStorage.getItem('noteData'))
// const noteJson = localStorage.getItem('noteData')
// const output = document.querySelector('.output')
// output.innerHTML = JSON.stringify(noteTones(toneNoteCsv), null, 2);

{ toneNoteCsv, noteTones }