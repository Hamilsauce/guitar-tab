...
export class GraphicObject {
  ...
  constructor(context, template, options) {
    if (!options.type) throw new Error('No type passed to constructor for ', this.constructor.name);
    
    const opts = options ? { ...GraphicObjectOptions, ...options } : GraphicObjectOptions;
    ...
    this.dom.querySelector('line').classList.remove('tab-line')
    this.dom.querySelector('line').setAttribute('stroke', '#FF0000')
    this.dom.querySelector('line').setAttribute('fill', '#FF0000')
  }
  ...
  static getSVGTemplate(svg, type, options = {}) {
    const template = (svg.dom || svg)
